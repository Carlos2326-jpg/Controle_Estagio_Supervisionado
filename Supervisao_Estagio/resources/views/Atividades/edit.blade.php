        ...
        <form method="POST" action="{{ route('atividades.update', $atividade) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <select name="solicitacao_id"
                        ...
                        @foreach($solicitacoes as $solicitacao)
                            <option value="{{ $solicitacao->id }}" {{ $atividade->solicitacao_id == $solicitacao->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="data_atividade" value="{{ old('data_atividade', $atividade->data_atividade->format('Y-m-d')) }}"
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="number" name="horas_computadas" value="{{ old('horas_computadas', $atividade->horas_computadas) }}" step="0.1" min="0.1"
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
